...
from . import new_pointnet2_utils
from typing import List
from pcdet.models.model_utils.weight_init import *
        

class NeighborVoxelSAModuleMSG_v2(nn.Module):
    ...
    def __init__(self, *, query_ranges: List[List[int]], radii: List[float], 
        ...
        for i in range(len(query_ranges)):
            ...
            nsample = nsamples[i]
            radius = radii[i]
            self.groupers.append(new_pointnet2_utils.VoxelQueryAndGrouping(max_range, radius, nsample))
            mlp_spec = mlps[i]
    ...
    def forward(self, xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt, \
        ...
        for k in range(len(self.groupers)):
            ...
            # torch.cuda.synchronize()
            # end_time = time.time()
            # print('time for voxel query:', end_time - start_time)
            # input()
